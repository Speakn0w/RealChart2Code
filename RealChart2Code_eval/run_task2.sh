...
# RealChart2Code Benchmark - Run Task2
...
python -u evaluate_task2.py \
  ...
  --generation_prompt_path "prompt_task2/benchmark_generate_prompt.txt" \
  --evaluation_prompt_path "prompt_task2/eval.txt" \
  --data_evaluation_prompt_path "prompt_task2/data_eval.txt" \
  --max_data_rows 5 \
  --timeout 120 \
  --max_retries 20 \
  --max_qps 15
