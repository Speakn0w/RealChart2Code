...
# RealChart2Code Benchmark - Run Task3
...
python -u evaluate_task3.py \
  ...
  --data_dir "data/data_task3.json" \
  --results_dir "results_task3" \
  --generation_prompt_path "prompt_task3/benchmark_generate_prompt.txt" \
  --evaluation_prompt_path "prompt_task3/eval.txt" \
  --user_history_generation_prompt_path "prompt_task1/benchmark_generate_prompt.txt" \
  --max_data_rows 5 \
  --timeout 120 \
  --max_retries 20 \
  --max_qps 15
