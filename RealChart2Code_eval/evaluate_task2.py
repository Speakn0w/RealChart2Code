...
def find_final_data_files(data_dir: Path, difficulty: str = None) -> List[Path]:
...
def create_user_prompt(category: str, data_info: Dict[str, str]) -> str:
...
def create_generation_request(category: str, data_info: Dict[str, str], image_path: str,
    ...
    text_prompt = create_user_prompt(category, data_info)
    image_base64 = encode_image_to_base64(image_path)
    user_content = [
        {
            "type": "text",
            "text": f'{text_prompt}.  Note that in this environment, the version of matplotlib is {matplotlib.__version__}, the version of seaborn is {seaborn.__version__}, the version of plotly is {plotly.__version__}, the version of bokeh is {bokeh.__version__}, the version of altair is {altair.__version__}.'
            # "text": text_prompt
        },
        {
            "type": "image_url",
...
def create_evaluation_request(difficulty: str, category: str, data_info: str, image_path:str,
...
def get_gt_code_path(image_path: Path) -> Path:
    """根据image_path获取对应的ground truth代码文件路径"""
    # 将*_plot_*.png改为*_code_*.py
    filename = image_path.name
    # 替换plot为code，png为py
    code_filename = filename.replace('_plot_', '_code_').replace('.png', '.py')
    return image_path.parent / code_filename


def read_gt_code(gt_code_path: Path) -> str:
    """读取ground truth代码文件"""
    try:
        with open(gt_code_path, 'r', encoding='utf-8') as f:
            return f.read().strip()
    except Exception as e:
        print(f"❌ Error reading ground truth code from {gt_code_path}: {e}")
        return ""
    
def create_data_evaluation_request(gt_code_path:Path, code_path: Path, data_evaluation_prompt: str, model_name: str) -> Dict:
    gt_code = read_gt_code(gt_code_path)
    gen_code = read_gt_code(code_path)
    ...
    return {
        "model": model_name,
        "messages": [
            {"role": "system", "content": data_evaluation_prompt},
            {"role": "user", "content": user_content}
        ],
        "max_tokens": 8192,
        "temperature": 0.0,
...
def parse_evaluation_response(response_content: str, data_response_content: str) -> Dict:
    # 首先按照原有逻辑解析主要的视觉评估结果
    evaluation = None
    
    # 尝试从response_content解析JSON
    try:
    ...
    # 确保evaluation是字典类型，不是字符串
    if isinstance(evaluation, str):
        evaluation = create_empty_evaluation()
    
    # 从data_response_content中提取<result>标签内容
    data_alignment = {'score': 0, 'reason': 'Data alignment not evaluated'}  # 默认值
    
    if data_response_content:
        # 尝试提取 <result> 标签内容
        result_match = re.search(r'<result>(YES|NO)</result>', data_response_content, re.IGNORECASE)
        if result_match:
            result = result_match.group(1).upper()
            if result == 'YES':
                data_alignment = {'score': 2, 'reason': 'Data alignment confirmed - identical data usage across all subplots'}
            else:
                data_alignment = {'score': 0, 'reason': 'Data alignment failed - different data usage detected'}
    
    # 将data_alignment添加到评估结果中
    evaluation['data_alignment'] = data_alignment
...
def create_empty_evaluation() -> Dict:
    return {
        ...
        'data_alignment': {'score': 0, 'reason': 'Data alignment failed'},
        'improvement_recommendations': ''
    }
    
def calculate_overall_score(evaluation: Dict) -> Dict:
    # Visual Structure Alignment scores (0-2 scale)
    ...
    # Data Alignment score (0-2 scale)
    data_alignment_score = evaluation.get('data_alignment', {}).get('score', 0)
    
    # Calculate totals
    visual_structure_total = sum(visual_structure_scores.values())
    execution_total = sum(execution_scores.values())
    data_alignment_total = data_alignment_score
    ...
    total_possible_data_alignment = 2  # 2 points max (0-2 scale)
    
    total_score = visual_structure_total + execution_total + data_alignment_total
    total_possible = total_possible_visual_structure + total_possible_execution + total_possible_data_alignment
    ...
    return {
        ...
        'data_alignment_total': data_alignment_total,
        'data_alignment_max': total_possible_data_alignment,
        ...
        'data_alignment_rate': data_alignment_total / total_possible_data_alignment if total_possible_data_alignment > 0 else 0,
...
def save_score_result(task_info: Dict, evaluation: Dict, raw_response: str, plot_file: Path, 
    ...
    if task_info['raw_generation_response']==None:
        task_info['raw_generation_response']=''
    ...
    result = {
        ...
        "raw_generation_response": task_info['raw_generation_response'],
...
def process_evaluation_responses(processed_tasks: List[Dict], evaluation_responses: List, data_evaluation_responses: List,
    ...
    for task_info in processed_tasks:
        try:
            if task_info.get('execution_success') and task_info.get('plot_file'):
                ...
                data_evaluation_response = data_evaluation_responses[eval_idx]
                eval_idx += 1
                ...
                if hasattr(data_evaluation_response, 'choices') and data_evaluation_response.choices:
                    data_eval_content = data_evaluation_response.choices[0].message.content
                elif isinstance(data_evaluation_response, dict) and 'choices' in data_evaluation_response:
                    data_eval_content = data_evaluation_response['choices'][0]['message']['content']
                else:
                    data_eval_content = ""
                
                
                evaluation = parse_evaluation_response(eval_content, data_eval_content)
            else:
                evaluation = create_empty_evaluation()
...
def generate_statistics(results_dir: Path, generation_model: str, evaluation_model: str) -> Dict:
    ...
    for score_file in score_files:
        ...
        if overall_score:
            score_data = {
                ...
                'data_alignment_total': overall_score['data_alignment_total'],
                'data_alignment_max': overall_score['data_alignment_max'],
                ...
                'data_alignment_rate': overall_score['data_alignment_rate'],
    ...
    def calc_stats(scores):
        ...
        data_alignment_scores = [s['data_alignment_total'] for s in scores]
        ...
        return {
            ...
            'data_alignment_avg': sum(data_alignment_scores) / len(data_alignment_scores),
            'data_alignment_max': scores[0]['data_alignment_max'],
            ...
            'data_alignment_perfect_rate': sum(1 for s in data_alignment_scores if s == scores[0]['data_alignment_max']) / len(data_alignment_scores),
...
def run_benchmark(args):
    ...
    data_evaluation_cache_file = results_dir / "data_evaluation_cache.jsonl"
    data_evaluation_client = FluxOpenAIChat(
        base_url=args.evaluation_base_url,
        api_key=args.evaluation_api_key,
        cache_file=str(data_evaluation_cache_file),
        max_retries=args.max_retries,
        max_qps=args.max_qps,
        max_qpm=args.max_qpm,
    )
    ...
    data_evaluation_prompt = read_system_prompt(args.data_evaluation_prompt_path)
    ...
    # task_infos = task_infos[:10]
    print(f"📤 Total loaded {len(task_infos)} tasks")
    ...
    if tasks_with_plots:
        ...
        for task_info in tasks_with_plots:
            ...
            request = create_evaluation_request(
                task_info['difficulty'], task_info['category'], task_info['data_info'], task_info['image_path'],
        ...
        data_evaluation_requests = []
        for task_info in tasks_with_plots:
            data_request = create_data_evaluation_request(
                get_gt_code_path(Path(task_info['image_path'])), get_gt_code_path(Path(task_info['plot_file'])), data_evaluation_prompt, args.evaluation_model_name
            )
            data_evaluation_requests.append(data_request)
        
        print(f"📤 Sending {len(data_evaluation_requests)} evaluation requests...")
        data_evaluation_responses = data_evaluation_client.request(data_evaluation_requests)
        
        
        print("💾 Processing evaluation responses and saving results...")
        final_results = process_evaluation_responses(processed_tasks, evaluation_responses, data_evaluation_responses,
    ...
    if statistics:
        ...
        if 'overall' in statistics and statistics['overall']:
            ...
            print(f"   Data Alignment Average Score: {overall['data_alignment_avg']:.2f}/{overall['data_alignment_max']}")
            ...
            print(f"   Data Alignment Perfect Rate: {overall['data_alignment_perfect_rate']:.1%}")
            ...
            for difficulty, stats in statistics['by_difficulty'].items():
                ...
                print(f"     Data Alignment Average Score: {stats['data_alignment_avg']:.2f}/{stats['data_alignment_max']}")
...
def main():
    ...
    parser.add_argument("--data_evaluation_prompt_path", type=str, 
                        default="prompts/data_eval.txt",
                        help="Evaluation system prompt file path")
