...
def create_generation_request(category: str, data_info: Dict[str, str], refine_before_image_path: str, image_path: str, improvement_suggestion: str, refine_before_code: str,
                            generation_prompt: str, user_history_generation_prompt:str, model_name: str) -> Dict:
    
    # user_prompt = create_user_prompt(category, instruction, data_info)
    refine_before_image_base64 = encode_image_to_base64(refine_before_image_path)
    ...
    messages = [
        {"role": "system", "content": user_history_generation_prompt},
        {"role": "user", "content": user1_content}
    ]
    
    assistant1_content = f"""Here's the Python code:
```python
{refine_before_code}
```
"""
    ...
    user2_content = [
        {
            "type": "text",
            "text": f"Look at this chart image and fix the errors in the code based on the instruction provided.\nCategory: {category}.\n\n**Instruction**: {improvement_suggestion}.\n\n**chart image**:"
        },
        {
            "type": "image_url",
            "image_url": {
                "url": refine_before_image_base64
            }
        }
    ]
    
    messages.extend([
            {"role": "assistant", "content": assistant1_content},
            {"role": "user", "content": user2_content}
        ])
    
    print(user_history_generation_prompt)
    print(user1_content[0])
    print(assistant1_content)
    print(user2_content[0])
...
def run_benchmark(args):
    ...
    user_history_generation_prompt = read_system_prompt(args.user_history_generation_prompt_path)
    ...
    try:
        ...
        for task_key, task_info in data.items():
            ...
            if 'refine_before_image' in task_info and task_info['refine_before_image']:
                task_info['refine_before_image'] = Path(task_info['refine_before_image'])
    ...
    for task_info in task_infos:
        request = create_generation_request(
            task_info['category'], task_info['data_info'], task_info['refine_before_image'], task_info['image_path'], task_info['improvement_suggestion'], task_info['refine_before_code'],
            generation_prompt, user_history_generation_prompt, args.generation_model_name
        )
...
def main():
    ...
    parser.add_argument("--user_history_generation_prompt_path", type=str, 
                        default="prompts/user_history_generate_code_prompt.txt",
                        help="User history code generation system prompt file path")
